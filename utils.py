...
class SeamImage:
    def __init__(self, img_path, vis_seams=True):
        ...
        print("Shape of self.rgb:", self.rgb.shape)
    ...
    def rgb_to_grayscale(self, np_img):
        ...
        """
        print("Shape of np_img:", np_img.shape)
        print("Converting RGB image to grayscale")
        r_weight, g_weight, b_weight = self.gs_weights
        gray_img = np_img[:, :, 0] * r_weight + np_img[:, :, 1] * g_weight + np_img[:, :, 2] * b_weight
    ...
    def calc_gradient_magnitude(self):
        ...
        # Print shape of self.gs
        print("Shape of self.gs:", self.gs.shape)
        # Extract dimensions of the image
        h, w, c = self.gs.shape

        # Initialize gradient magnitude image with zeros
        gradient_magnitude = np.zeros((h, w), dtype=np.float32)

        # Compute gradient magnitude for each pixel
        for i in range(1, h - 1):
            for j in range(1, w - 1):
                # Compute horizontal and vertical gradients using central difference
                dx = np.abs( self.gs[i + 1, j ] - self.gs[i, j ])
                dy = np.abs(self.gs[i , j+1] - self.gs[i , j])

                # Compute gradient magnitude using the Euclidean norm
                gradient_magnitude[i, j] = np.sqrt(dx ** 2 + dy ** 2)

        # Normalize gradient magnitude values to be in the range [0, 1]
    ...
    def rotate_mats(self, clockwise=True):
        if clockwise:
            # Rotate both matrices clockwise
            self.rgb = np.rot90(self.rgb, k=3)
            self.seams_rgb = np.rot90(self.seams_rgb, k=3)
        else:
            # Rotate both matrices counter-clockwise
            self.rgb = np.rot90(self.rgb)
...
class VerticalSeamImage(SeamImage):
    ...
    def calc_M(self):
        ...
        """
        print("Calculating energy matrix M")
        # Initialize energy matrix M with the same shape as the grayscale image
        M = self.E.copy()

        # Normalize energy values to be in the range [0, 1]
        normalized_energy = M / np.max(M)

        # Iterate over rows from the second row to the last row
        for i in range(1, self.h - 1):
            # Calculate the cumulative energy for each pixel in the current row
            for j in range(1, self.w - 1):
                # Calculate the costs for the three possible paths
                cost_left = M[i - 1, j - 1] 
                cost_mid = M[i - 1, j]
                cost_right = M[i - 1, j + 1]

                # Update the energy of the current pixel as the sum of its own energy and the minimum of the three costs
                M[i, j] = self.E[i,j] + min(cost_left, cost_mid, cost_right) 
    ...
    def seams_removal_vertical(self, num_remove):
        """ Remove num_remove vertical seams from the image.

        Parameters:
            num_remove (int): Number of vertical seams to be removed.
        """
        og_image = self.rgb.copy()
        for _ in range(num_remove):
            
            self.rgb = self.carve_column(self.rgb)
    ...
    def minimum_seam(self):
        ...
        for i in range(1, r):
            for j in range(0, c):
                ...
                if j == 0:
                    idx = np.argmin(M[i - 1, j:j + 2])  # Include 3 adjacent cells
                    backtrack[i, j] = idx + j
                    min_energy = M[i - 1, idx + j]
                else:
                    idx = np.argmin(M[i - 1, max(0, j - 1):j + 2])  # Include 3 adjacent cells
    ...
    def carve_column(self, img):
        ...
        deleted_seams = []  # List to store the indices of deleted seams

        for i in reversed(range(r)):
            # Mark the pixels for deletion
            mask[i, j] = False
            deleted_seams.append((i, j))  # Store the indices of deleted seam
            j = backtrack[i, j]
        
        for i, j in deleted_seams:
            #print(f"RGB value of the deleted pixel before: {self.seams_rgb[i, j]}")
            self.seams_rgb[i, j] = [1, 0, 0]  # Red color
            #print(f"RGB value of the deleted pixel after: {self.seams_rgb[i, j]}")
        ...
        # Update dimensions after removing one column
        self.w -= 1
